...
const ReservationUser = ({ registrations, onEditRegistration, deleteRegistration }) => {

  const [localRegistrationsUser, setLocalRegistrationUser] = useState(registrations)

  console.log(localRegistrationsUser)
 
  useEffect(() => {
    if (registrations) {
      setLocalRegistrationUser(registrations); 
    }
  }, [registrations]);

  return (
    <div className="table-container">
      <h3>Prenotazioni eventi</h3>
      <table className="table">
        ...
        <tbody>
          {localRegistrationsUser.map((registration, index) => (
            <tr key={registration._id}>
              <td>{index + 1}</td>
              <td>{registration.userId?.name} {registration.userId?.surname}</td>
              <td>{registration.userId?.email}</td>
              <td>{registration.eventId?.name}</td>
              <td>{new Date(registration.eventId?.date).toLocaleDateString('it-IT')}</td>
